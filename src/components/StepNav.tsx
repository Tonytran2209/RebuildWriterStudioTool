...
export default function StepNav({ currentStep, onStepChange, currentModel, syncStatus = 'idle' }: Props) {
  ...
  return (
    <header className="bg-white border-b border-slate-200/80 px-5 py-3 flex items-center justify-between shrink-0 shadow-sm z-10">
      {/* Step tabs */}
      <div className="bg-[#eaedf3] p-1.5 rounded-2xl flex items-center space-x-1 shadow-sm">
        {[1, 2, 3, 4].map(step => {
          ...
          return (
            <button
              ...
              className={`px-3.5 py-2 text-xs font-semibold rounded-xl transition-all flex items-center space-x-2 ${
                active
      ...
      <div className="flex items-center space-x-3 text-xs">
        {currentModel && (
          <span className="bg-indigo-50 border border-indigo-100 text-indigo-700 font-medium px-2.5 py-1.5 rounded-xl flex items-center space-x-1.5">
        ...
        <div className="flex items-center space-x-1.5">
          ...
          <span className="font-medium text-[11px] text-slate-500">{sync.label}</span>
        </div>
      </div>
    </header>
  );
