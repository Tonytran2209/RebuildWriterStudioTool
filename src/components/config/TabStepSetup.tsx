...
export default function TabStepSetup({ config, files, articles, onChange }: Props) {
  ...
  return <div className="space-y-4">
    <div className={`rounded-2xl border p-3.5 flex items-center gap-3 ${backendOk ? 'bg-emerald-50 border-emerald-200' : backendOk === false ? 'bg-red-50 border-red-200' : 'bg-slate-50 border-slate-200'}`}>
    ...
    {[1, 2, 3, 4].map(step => { const calls = usageSummary[String(step)] ?? []; const totalTokens = calls.reduce((sum, call) => sum + Number(call.totalTokens ?? 0), 0); return <div key={step} className="rounded-2xl border border-slate-200 bg-slate-50 p-4 flex flex-col sm:flex-row sm:items-center gap-3">
