...
export default function WorkflowRulesPanel({ config, files, onChange }: Props) {
  ...
  return <div className="workflow-rules-panel space-y-7">
    ...
    <section>
      ...
      {phase.id === 'knowledge' && <div className="knowledge-source-summary mb-4 grid gap-2 sm:grid-cols-2"><div className="rounded-lg bg-slate-50 px-4 py-3"><p className="text-xs text-slate-400">Knowledge Base</p><p className="mt-1 text-base font-medium text-slate-800">{readyKb} {tr('nguồn sẵn sàng', 'ready sources')}</p></div><div className="rounded-lg bg-slate-50 px-4 py-3"><p className="text-xs text-slate-400">Reference guides</p><p className="mt-1 text-base font-medium text-slate-800">{readyGuides} {tr('nguồn sẵn sàng', 'ready sources')}</p></div></div>}
      ...
      <div className="space-y-5">
        {definitions.map(rule => {
          ...
          const promptGroups = [
            { id: 'system', label: tr('System prompt · Bắt buộc', 'System prompt · Required'), content: preview.systemPrompt },
            { id: 'guidance', label: tr('Task guidance · Hướng dẫn', 'Task guidance · Guided'), content: preview.taskGuidance },
          ...
          return <section key={rule.id} className="workflow-rule-group overflow-hidden rounded-xl border border-slate-200 bg-white">
            ...
            <footer className="border-t border-slate-200 bg-slate-50 px-4 py-3"><div className="flex items-center justify-between gap-3"><span className="text-xs font-medium text-slate-500">{tr('Prompt đang áp dụng', 'Effective prompt')}</span>{customized && <button onClick={() => resetRule(rule.id)} className="text-xs text-slate-500">{tr('Reset toàn bộ rule', 'Reset rule')}</button>}</div><details className="effective-prompt mt-2 min-w-0"><summary className="cursor-pointer text-xs text-slate-500">{tr('Xem nội dung prompt', 'View prompt content')}</summary><div className="mt-3 space-y-3">{promptGroups.length ? promptGroups.map(group => <section key={group.id} className="prompt-group overflow-hidden rounded-lg border border-slate-200"><div className="prompt-group-label border-b border-slate-200 px-3 py-2 text-xs font-medium text-slate-600">{group.label}</div><pre className="max-h-64 overflow-auto whitespace-pre-wrap p-3 font-mono text-xs leading-6 text-slate-600">{group.content}</pre></section>) : <p className="rounded-lg border border-slate-200 px-3 py-3 text-xs text-slate-500">{tr('Rule không áp dụng cho chế độ này.', 'Rule is not active in this mode.')}</p>}</div></details></footer>
          </section>;
        })}
      </div>
    </section>
