...
export const UNIVERSAL_QUALITY_VERSION = 1;
...
export function deterministicQualityChecks(article: Article, draft: string, inventory: WebsiteContentRecord[] = [], sourceNames: string[] = []): QualityGateCheck[] {
  ...
  const checks: QualityGateCheck[] = [
    ...
    { id: "word-budget", label: "English word budget", kind: "deterministic", status: count >= Math.floor(target * .9) && count <= target ? "pass" : "fail", reason: `${count}/${target} English words.`, autoFixAllowed: true },
