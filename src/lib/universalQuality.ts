...
export const UNIVERSAL_QUALITY_VERSION = 4;
...
export function deterministicQualityChecks(article: Article, draft: string, effectiveWordLimit: number, inventory: WebsiteContentRecord[] = [], sourceNames: string[] = []): QualityGateCheck[] {
  ...
  const checks: QualityGateCheck[] = [
    ...
    { id: "word-budget", label: "English word budget", kind: "deterministic", status: count >= 800 && count <= hardLimit ? "pass" : "fail", reason: `${count} English words; required minimum 800 and configured hard limit ${hardLimit}.`, autoFixAllowed: true },
